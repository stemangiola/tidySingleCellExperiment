...
test_that("arrange",{
  ...
  tt_pca_aranged = tt %>% arrange(groups) %>% scater::logNormCounts() %>% scater::runPCA()
  tt_pca = tt %>% scater::logNormCounts() %>% scater::runPCA()
...
test_that("bind_rows",{
  ...
  expect_equal(
    tt_bind %>% select(cell) %>% tidySCE:::to_tib() %>% dplyr::count(cell) %>% dplyr::count(n) %>% nrow,
    1
  )
...
test_that("select",{
  ...
  expect_equal(   tt %>% select(cell, orig.ident ) %>% class %>% as.character,    "tidySCE"  )
