data("pbmc_small")
set.seed(2023-08-29)
rand_mat <- matrix(data = runif(n = length(LETTERS) * dim(pbmc_small)[[2]]), ncol = dim(pbmc_small)[[2]])
colnames(rand_mat) <- rownames(colData(pbmc_small))
rownames(rand_mat) <- LETTERS
altExps(pbmc_small)[["ADT"]] <- SingleCellExperiment(assays = list(normcounts = rand_mat))
df <- pbmc_small
